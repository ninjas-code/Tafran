				...
				<div className="App">
					<header className="App-header">
						<form>
							...
							<button className="search1" onClick={this.sendPriceToServer.bind(this)}>
							...
							{this.state.dispalyMealList ? (
								<MealsList
									...
									sendRestNameAndPrice={this.sendRestNameAndPrice.bind(this)}
								/>
							) : null}

							{this.state.displaRestList ? <Resturant resturants={this.state.resturants} /> : null}
						</form>
					</header>
				</div>
			</BrowserRouter>
				...
				{this.props.meals.length > 0 ? (
					<table className="Tabel">
						<thead>
							<tr>
								<th className="MealsR">
									<i class="fas fa-utensils" />TheRestRunt
								</th>
							...
							{this.props.meals.map((meal, i) => {
								return (
									<tr
										key={i}
										value={meal}
										onClick={(event) => this.props.sendRestNameAndPrice(event, meal)}
									>
					...
					<table>
						...
						<tbody>
							{this.props.resturants.map((resturant, i) => {
								return (
									<tr key={i} onClick={() => console.log('this is' + resturant.restName)}>
